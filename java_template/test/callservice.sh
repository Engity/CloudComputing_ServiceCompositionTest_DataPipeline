...
time output=`aws lambda invoke --invocation-type RequestResponse --function-name switchBoard --region us-east-2 --payload $jsonT --cli-connect-timeout 900 --cli-read-timeout 900 /dev/stdout | head -n 1 | head -c -2 ; echo`
echo ""
echo "JSON RESULT:"
echo $output | jq
...
time output=`aws lambda invoke --invocation-type RequestResponse --function-name processSalesData --region us-east-2 --payload $jsonL /dev/stdout | head -n 1 | head -c -2 ; echo`
